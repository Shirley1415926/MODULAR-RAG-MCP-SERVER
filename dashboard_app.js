...
function renderFeedback(){
  ...
  const reasons=['Emergency / Unforeseen Obligations','Change of Mind','Financial Issues','Patient Anxiety / Resistance','Unknown'], counts=reasons.map(r=>s.c.filter(a=>a.cancellation_reason===r).length);
  draw('reasonChart',reasons,[{label:'Cancelled appointments',data:counts,backgroundColor:['#9497dd','#e76064','#474b9e','#f2a645','#fae8c7']}],'doughnut');
  $$('.donut-legend .legend-row').forEach((el,i)=>el.innerHTML=`<span class="swatch" style="background:${palette[i]}"></span>${esc(reasons[i])} · ${counts[i]} (${pc(rate(counts[i],s.c.length))})`);
