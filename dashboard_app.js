...
function renderFeedback(){
  ...
  $('#syntheticDataSummary').textContent=`${dateLabel(f.start)} – ${dateLabel(f.end)} · ${surveys.length} responses`;
  ...
  ['cancelChart','noShowChart'].forEach((id,i)=>draw(id,buckets.map(bucketLabel),[{label:i?'No-show rate (%)':'Cancellation rate (%)',data:bucketStats.map(x=>i?x.nr:x.cr),borderColor:i?'#777bc6':'#e97879',backgroundColor:i?'#777bc6':'#e97879',tension:.25,cubicInterpolationMode:'monotone',pointRadius:5,pointHoverRadius:7,borderWidth:2.5}],'line',{interaction:{mode:'index',intersect:false},layout:{padding:{left:4,right:12,top:8}},scales:{x:{grid:{display:false},ticks:{autoSkip:false,maxRotation:0,font:{size:10},padding:8}},y:{beginAtZero:true,max:ceiling,grid:chartGrid,ticks:{maxTicksLimit:6}}},plugins:{legend:{display:false},tooltip:{callbacks:{title:items=>{const x=buckets[items[0].dataIndex];return x.start+' → '+x.end;},afterLabel:ctx=>{const b=bucketStats[ctx.dataIndex];return `${i?b.n.length:b.c.length} / ${b.all.length} attempts${b.all.length<5?' · low sample':''}`;}}}}}));
  $$('.fb-charts .line-panel').forEach((panel,i)=>{let caption=panel.querySelector('.chart-scope');if(!caption){caption=document.createElement('p');caption.className='chart-scope';panel.append(caption);}caption.textContent=calendarBuckets.length===7?'Service days only · non-working days excluded':'Grouped by the date ranges shown';});
  ...
  $('#feedbackInsights').innerHTML=`<div class="insights-head"><h2>Key Operational Insights</h2><button class="ai-generate" data-ai-scenario="feedback-overview">View all feedback themes & evidence</button></div>`+
    [[`Cancellation: ${pc(s.cr)}`,`${s.c.length} cancellations, ${delta(s.cr,p.cr,true)} compared with the previous period. Review the main cancellation reasons before choosing an action.`,'feedback-cancellation'],
      ...
      [t.insufficient?'More feedback needed':`${t.total} patient responses`,t.insufficient?'There is not enough feedback to identify a recurring theme yet.':`Most frequently mentioned: ${t.themes[0]?.label}. Review comments alongside the scores before deciding what to improve.`,'feedback-overview']].map(([title,copy,scenario])=>`<div class="insight"><div class="insight-copy"><b>${esc(title)}</b><p>${esc(copy)}</p></div><button class="insight-evidence" data-ai-scenario="${scenario}">View details →</button></div>`).join('');
...
function init(){
  ...
  $('#portfolioBanner').innerHTML='Demo · Fictional data';$('#portfolioBanner').classList.add('show');
  ...
  $$('.middle-card h2')[1].insertAdjacentHTML('afterend','<p class="middle-subtitle">Reported impact · 1 = minor, 5 = major</p>');
